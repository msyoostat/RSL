...
library(plgp)
set.seed(1)
load("/Users/myungsooyoo/Desktop/My_stuff/research/emulator/AML_project_important-20230821T155429Z-001/AML_project_important/zero_out/data_all_cities_py_15june.RData")

test_data_x <- as.matrix(test.data.lithk)
train_data_x <- as.matrix(train.data.lithk)
val_data_x <- as.matrix(val.data.lithk)
...
# read pc result for input from python ------------------------------------
train_data_x<-read.table("/Users/myungsooyoo/Desktop/My_stuff/research/emulator/python/darwin/final/previous/previous_final/train_data_x.txt", header = FALSE, sep = ",", dec = ",")
test_data_x<-read.table("/Users/myungsooyoo/Desktop/My_stuff/research/emulator/python/darwin/final/previous/previous_final/test_data_x.txt", header = FALSE, sep = ",", dec = ",")
val_data_x<-read.table("/Users/myungsooyoo/Desktop/My_stuff/research/emulator/python/darwin/final/previous/previous_final/val_data_x.txt", header = FALSE, sep = ",", dec = ",")

train_data_x<-as.matrix(train_data_x)
test_data_x<-as.matrix(test_data_x)
val_data_x<-as.matrix(val_data_x)
colnames(train_data_x)<-NULL
colnames(test_data_x)<-NULL
colnames(val_data_x)<-NULL

# scratch -----------------------------------------------------------------
## first start with this (covariance function: exp(-D/theta) + nugget)
eps <- sqrt(.Machine$double.eps)
D <- distance(train_data_x)
DXX <- distance(test_data_x)
DX <- distance(test_data_x, train_data_x)
...
for(j in 1: ncol(test_data_y)){
  ...
  K <- exp(- D/(10000*out$par[1]) ) + diag(out$par[2], nrow(train_data_x))
  Ki <- solve(K)
  #### tau square estimate
  tau2hat <- drop(t(train_data_y[,j]) %*% Ki %*% train_data_y[,j] / nrow(train_data_x))
