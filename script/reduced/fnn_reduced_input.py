...
result = pyreadr.read_r('regional_cities_train_test_val_py.RData') 
#result = pyreadr.read_r('/Users/myungsooyoo/Desktop/My_stuff/research/emulator/AML_project_important-20230821T155429Z-001/AML_project_important/regionalization/regional_cities_train_test_val_py.RData') 
print(result.keys(),flush=True)
test_data_lithk_region_mean_x= result['test.data.lithk.region.mean']
train_data_lithk_region_mean_x= result['train.data.lithk.region.mean']
val_data_lithk_region_mean_x= result['val.data.lithk.region.mean']
...
test_data_lithk_region_mean_x=test_data_lithk_region_mean_x.to_numpy()
test_data_y=test_data_y.to_numpy()

train_data_lithk_region_mean_x=train_data_lithk_region_mean_x.to_numpy()
train_data_y=train_data_y.to_numpy()
val_data_lithk_region_mean_x=val_data_lithk_region_mean_x.to_numpy()
...
scaler_x.fit(train_data_lithk_region_mean_x)

train_data_lithk_region_mean_x= scaler_x.fit_transform(train_data_lithk_region_mean_x)
val_data_lithk_region_mean_x= scaler_x.fit_transform(val_data_lithk_region_mean_x)
test_data_lithk_region_mean_x= scaler_x.fit_transform(test_data_lithk_region_mean_x)


# np.savetxt('train_data_lithk_region_mean_x.txt', train_data_lithk_region_mean_x, delimiter=',')
# np.savetxt('val_data_lithk_region_mean_x.txt', val_data_lithk_region_mean_x, delimiter=',')
# np.savetxt('test_data_lithk_region_mean_x.txt', test_data_lithk_region_mean_x, delimiter=',')

train_data_lithk_region_mean_x = torch.from_numpy(train_data_lithk_region_mean_x)
val_data_lithk_region_mean_x = torch.from_numpy(val_data_lithk_region_mean_x)
test_data_lithk_region_mean_x = torch.from_numpy(test_data_lithk_region_mean_x)

train_data_y = torch.from_numpy(train_data_y)
val_data_y = torch.from_numpy(val_data_y)

train_data_lithk_region_mean_x=train_data_lithk_region_mean_x.type('torch.FloatTensor')
test_data_lithk_region_mean_x=test_data_lithk_region_mean_x.type('torch.FloatTensor')
val_data_lithk_region_mean_x=val_data_lithk_region_mean_x.type('torch.FloatTensor')
...
#%% FNN
input_size_x= train_data_lithk_region_mean_x.shape[1]
...
train_dataset = TensorDataset(train_data_y,train_data_lithk_region_mean_x)
val_dataset = TensorDataset(val_data_y,val_data_lithk_region_mean_x)
...
cvae.eval()
val_data_x= val_data_lithk_region_mean_x.to(device)
...
test_data_x= test_data_lithk_region_mean_x.to(device)
...
import pickle
result = {'predictions_val':predictions_val,'costval_training':costval_training, 'costval_validation':costval_validation, 'S_square_val':S_square_val, 'intercept_val':intercept_val,'slope_val':slope_val,
...
with open('fnn_reduced_input.pickle', 'wb') as f:
